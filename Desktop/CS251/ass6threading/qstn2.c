...
void calculation(AC* account, txn* trans, int id)
{
   ...
   else 
   {
	   ...
	   {
		   if (new->opt==1)
			   (account+id)->bal=(account+id)->bal + 0.99*new->amount;

		   if (new->opt==2)
			   (account+id)->bal=(account+id)->bal-1.01*new->amount;
		   else if(new->opt==3)
...
void *threading(void *arg)
{
  ...
  int skip=params->skip;
	...
	{
		calculation(params->acc,(params->acc+j)->next,j);
			j=j+skip;
	}
        return NULL;
}
void insertion(AC *account, int acc1,int opcode,int amount)
{
		
    if(account!=NULL){
		 txn *curr;
     txn *new=(txn*)malloc(sizeof(txn));
		//new=malloc     
		  if ((account+acc1-1001)->next==NULL)
		      (account+acc1-1001)->next=new;
		  else 
			{
        curr = (account+acc1-1001)->next;
...
{
	fscanf(accnt,"%d %lf\n", &(account+i)->acc_no,&(account+i)->bal);
}
